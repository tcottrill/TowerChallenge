	...
	for (int i = 0; i < BUFFER_SIZE; i++)
	{
		...
		for (std::list<int>::iterator it = audio_list.begin(); it != audio_list.end(); ++it)
		{
			if (channel[*it].state == SOUND_PLAYING)
			{
				...
				//wrlog("Audio sample here is %d, state is %d", channel[a].loaded_sample_num, channel[a].state);
				//if (p.bitPerSample == 16)
				//{

				channel[*it].pos += p->channels; //If it's stereo only play the left channel.

				if (channel[*it].pos >= p->sampleCount)
				{
					channel[*it].pos = 0;
		...
		if (fmix)
		{
			if (fmix > 32767) { fmix = 32767; wrlog("truncating high"); };
			if (fmix < -32768) { fmix = -32768; wrlog("truncating low"); }
		}
